...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    if(isAtLeastiOS11){
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 110000
        //sessionOnly cookies may not sync , copy all cookies to WKHTTPCookieStore
...
- (void)createViews
{
    ...
    // splicing userAgent
    if (_browserOptions.appenduseragent != nil) {
        [self.webView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id result, NSError *error) {
            NSString *userAgent = result;
            NSString *newUserAgent = [userAgent stringByAppendingFormat: @" %@", _browserOptions.appenduseragent];
   ...
   if (@available(iOS 11.0, *)) {
    ...
    // toolbarcolor default white
    NSString* toolbarcolor = @"#ffffff";
    if (_browserOptions.toolbarcolor != nil) { // Set toolbar color if user sets it in options
        toolbarcolor = _browserOptions.toolbarcolor;
    }
    self.toolbar = [[TranslucentToolbar alloc]initWithFrame: toolbarFrame];
    self.toolbar.backgroundColor = [self colorFromHexString: toolbarcolor];
    ...
    NSString* defaultColorString = @"#000000";
    NSString* frontArrowString = NSLocalizedString(@"►", nil); // create arrow from Unicode char
    self.forwardButton = [[UIBarButtonItem alloc] initWithTitle:frontArrowString style:UIBarButtonItemStylePlain target:self action:@selector(goForward:)];
    self.forwardButton.enabled = YES;
    self.forwardButton.imageInsets = UIEdgeInsetsZero;
    self.forwardButton.tintColor = [self colorFromHexString:defaultColorString];
    ...
    self.backButton.tintColor = [self colorFromHexString:defaultColorString];
    if (_browserOptions.navigationbuttoncolor != nil) { // Set button color if user sets it in options
...
- (void)setCloseButtonTitle:(NSString*)title : (NSString*) colorString : (int) buttonIndex
{
    ...
    // button color default black
    NSString* defaultColorString = @"#000000";
    self.closeButton.tintColor = colorString != nil ? [self colorFromHexString:colorString] : [self colorFromHexString: defaultColorString];
